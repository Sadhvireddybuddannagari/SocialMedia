          ...
          <div className='flex items-end justify-end pb-2'>
            {loading ? <Loading /> : (
              <CustomButton
                type="submit"
                title='submit'
                containerStyle='bg-[#0444a4] text-white py-1 px-3 rounded-full text-sm'
              />
            )}
          </div>
        </div>
    ...
    return (
      <div className='mb-2 bg-primary p-4 rounded-xl'>
        <div className='flex gap-3 items-center mb-2'>
          ...
          <div className='w-full flex justify-between'>
            <div>
              ...
              <span className='md:hidden flex text-ascent-2 text-xs'>{moment(post?.createdAt ?? '2023-05-25').fromNow()}</span>

            </div>
            <span className=' hidden md:flex text-ascent-2'>{moment(post?.createdAt ?? '2023-05-25').fromNow()}</span>
          </div>
        </div>
        <div>
          <p className='text-ascent-2'>
            {showAll === post?._id ? post?.description : `${post?.description.slice(0, 300)}...`}
            {post?.description?.length > 310 && (
              showAll === post?._id ? (
                <span
        ...
        {showComments === post?._id && (
          <div className='w-full mt-4 border-t border-[#6666645] pt-4'>
            ...
            {loading ? (
              ...
              comments.length > 0 ? (
                  ...
                  <div className='w-full py-2' key={comment?._id}>
                    ...
                    <div className='ml-12'>
                      ...
                      {replyComments === comment?._id && (
                        <CommentForm user={user} replyAt={comment?.from} id={comment?._id} getComments={() => getComments(post?._id)} />
                      )}
                    </div>
                    <div className='py-2 px-8 mt-6'>
                      {comment?.replies?.length > 0 && (
                        <p
                          ...
                          onClick={() => setShowReply(showReply === comment?.replies?._id ? null : comment?.replies?._id)}
                        >
                ...
                <span className='flex text-sm py-4 text-ascent-2 text-center'>No Comments, be the first to comment</span>
              )
            )}
          </div>
        )}
